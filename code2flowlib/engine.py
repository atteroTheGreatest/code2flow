...
The implementation files (javascript.py and python.py) subclass every one of
these classes sometimes replacing functions and sometimes adding
completely new functions

This way, we can share a lot of code between languages while still preserving
...
def generateEdges(nodes):
    ...
    This is in the global scope because edges can exist between any node and
...
class Node(object):
    ...
    def __init__(self, name, definitionString, source, parent,
        ...
        # The name pattern which is found by others eg. node()
        self.pattern = re.compile(r"(?:\W|\A)(%s)\s*\(" % self.name,
                                  re.MULTILINE)

        self.determineNodeType()  # Init node, etc.

        # The pattern to search for when the other node
        # is in the same scope e.g. self.node()
    ...
    def generateSameScopePatterns(self):
        return [re.compile(r"(?:\W|\A)%s\.%s\s*\(" % (self.sameScopeKeyword,
                                                      self.name),
                re.MULTILINE | re.DOTALL)]

    def generateAnyScopePatterns(self):
        return [
            re.compile(r"(?:[^a-zA-Z0-9\.]|\A)%s\s*\(" % (self.getFullName()),
                       re.MULTILINE | re.DOTALL)]
    ...
    def determineNodeType(self):
        '''
        Dummy meant to be subclassed if we do extra calculations
    ...
    def isExtraneous(self, edges=None):
        ...
        Will contain logic that will determine whether
        this node can be removed during trimming
        '''
        return False
